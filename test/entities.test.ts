...
import { ChainId, WXRP as _WXRP, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const CHAIN_ID = ChainId.XRPL_EVM_SIDECHAIN_DEVNET
const WXRP = _WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WXRP, decimalize(1234, WXRP.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WXRP]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WXRP)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WXRP, decimalize(10, WXRP.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WXRP, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WXRP, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WXRP, '1234567000000000000000')
