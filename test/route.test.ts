...
describe('Route', () => {
  ...
  const weth = WXRP[ChainId.XRPL_EVM_SIDECHAIN]
  ...
  it('constructs a path from the tokens', () => {
    ...
    expect(route.chainId).toEqual(ChainId.XRPL_EVM_SIDECHAIN)
  })
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], XRP[ChainId.XRPL_EVM_SIDECHAIN])
  ...
  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, XRP[ChainId.XRPL_EVM_SIDECHAIN])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
