...
import { ChainId, XRP, CurrencyAmount, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WXRP } from '../src'
...
describe('Trade', () => {
  ...
  const token2 = new Token(ChainId.XRPL_EVM_SIDECHAIN_DEVNET, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.XRPL_EVM_SIDECHAIN_DEVNET, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]),
      CurrencyAmount.ether(JSBI.BigInt(100), ChainId.XRPL_EVM_SIDECHAIN_DEVNET),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET], token0),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]),
      CurrencyAmount.ether(JSBI.BigInt(100), ChainId.XRPL_EVM_SIDECHAIN_DEVNET),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
  })
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
      expect(result[0].route.path).toEqual([WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
      expect(result[1].route.path).toEqual([WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      const result = Trade.bestTradeExactIn(
        ...
        XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(token3)
      expect(result[0].route.path).toEqual([token3, token0, WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]])
      expect(result[0].outputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WXRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET]])
      expect(result[1].outputAmount.currency).toEqual(XRP[ChainId.XRPL_EVM_SIDECHAIN_DEVNET])
    })
  })
